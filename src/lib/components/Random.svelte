...
<svelte:window
	on:keydown|preventDefault={(e) => {
        if (e.code === 'Space') {
            data.set(getRandomSpecial());
        }
	}}
/>

<button
	class="p-2 rounded-xl bg-slate-700 hover:bg-slate-700/80 w-fit transition active:translate-y-0.5"
	on:click|preventDefault={data.set(getRandomSpecial())}
	><svg
